...
const createTask = async (posterId, taskData) => {
  ...
  // Calculate total cost (reward * workers + 10% platform fee)
  const platformFee = (reward * maxWorkers * parseFloat(process.env.PLATFORM_FEE_PERCENT || 10)) / 100;
  const totalCost = reward * maxWorkers + platformFee;

  // Lock funds immediately on task creation (escrow)
  const escrowResult = await lockFundsForTask(posterId, 'PENDING', totalCost, currency);
